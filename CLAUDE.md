...
#### X活動
- **x-writer**: X投稿を生成（`x-voice` 自動適用、カテゴリ判定、スレッド対応）
- **x-voice**: X投稿の文体・トーン定義（自動適用、直接呼出不可）
- **x-reviewer**: X投稿の5軸品質チェック（トーン/情報密度/セキュリティ/フォーマット/連携）
- **x-strategy**: 投稿テーマ提案・週次スケジュール・コンテンツミックス確認
